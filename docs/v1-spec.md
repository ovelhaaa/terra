...
Fases sugeridas
Engine mínima: build web, arquivo passando por gain, param em tempo real, sem alocação por callback.
Patch serial: add/remove/reorder, save/load JSON, sincronização UI/engine.
Módulos base: gain/filter/delay/overdrive/chorus/reverb/lfo com schema + wet/dry + bypass.
UI de nodes: drag, ordem visual = ordem de processamento, inspector dinâmico, save/load integrado.
Métricas: painel estável com callback avg/peak, latência e memória.
